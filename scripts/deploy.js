...
function updateLocalConfigs(gcfUrl) {
...
`;
  ...
  console.log(`📝 Updated ${configPath} with GCF backend URL.`);
  ...
  if (fs.existsSync(manifestPath)) {
    ...
    // Comment out dev url, insert file: "bundle.js"
    manifest = manifest.replace(
      /url:\s*"https:\/\/localhost:8080\/bundle\.js"/g,
      `# url: "https://localhost:8080/bundle.js"\n  file: "bundle.js"`
    );

    // Update external_api_urls if GCF domain is not already in there
    const urlPattern = new RegExp(`"${gcfUrl}"`, 'g');
    ...
    console.log(`📝 Updated ${manifestPath} for production.`);
  }
...
async function main() {
  ...
  updateLocalConfigs(gcfUrl);
  await configureLookerAttribute(config, gcfUrl);

  console.log('\n🎉 Deployment and setup completed successfully! 🎉');
  console.log('----------------------------------------------------');
  console.log('Next Steps:');
  console.log('1. Commit and push the updated frontend Looker project files (like manifest.lkml and bundle.js) to your Looker project repository.');
  console.log('2. Confirm that model configurations in your Looker instance are set up to grant access to the extension.');
  console.log('3. Open Looker, load the Nano Admin extension, and test the functionality.');
  console.log('----------------------------------------------------');
}
